...
class HealthAssetPreviewService {
  ...
  Future<void> preview(BuildContext context, HealthAsset asset) async {
    try {
      ...
      if (handledBySystem) {
        if (context.mounted) {
          ScaffoldMessenger.of(
            context,
          ).showSnackBar(SnackBar(content: Text('已在系统中打开 ${file.path}')));
        }
        return;
      }
  ...
  Future<bool> _tryOpenWithSystemExplorer(File file) async {
    try {
      ...
      if (Platform.isWindows) {
        final result = await Process.run('explorer', ['/select,${file.path}']);
